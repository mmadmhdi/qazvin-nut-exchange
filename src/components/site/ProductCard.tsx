  ...
  return (
    ...
    >
      ...
      <div className="mt-6 flex items-end justify-between hairline-t pt-4">
        <div>
          <div className="text-[10px] tracking-widest uppercase text-muted-foreground">{t("products.todayPrice")}</div>
          <div className={`font-display num-fa text-olive-deep ${size === "lg" ? "text-3xl" : "text-xl"}`}>
            {formatPrice(product.price)}
            <span className="text-xs text-muted-foreground mx-2">{l.unit}</span>
          </div>
        </div>
